...
등록 후 `tools/list`를 호출하면 아래 4개 툴이 보인다.
...
- `q` (string): 키워드. `type`/`country`/`city`/`place` 중 아무 필드에나 부분일치하면 걸린다. 정확한 필터링 조건이 애매할 때 우선 이걸로 넓게 검색.
...
- 필수 필드 누락, `type` 오탈자, `minPeople > maxPeople` 등 검증 실패 시 에러 메시지로 구체적인 사유가 온다 — 그 메시지를 그대로 사용자에게 보여주면 된다
...
**시나리오 C — 공고 등록**
1. 사용자: "내일 부산 해운대 택시 같이 탈 사람 구하는 공고 올려줘, 최대 3명, 3만원"
2. 필요한 필드(country, city, place, date, time, minPeople, maxPeople, price, agentId)를 사용자에게 확인/보완
3. `create_posting({...})` 호출, 생성된 `id`를 사용자에게 알려줌
