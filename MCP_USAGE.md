...
**실행 규칙:** 필수 파라미터와 사용자 제약이 모두 확보되면 **최종 확인을 다시 묻지 않고 바로 등록한다.** 정보는 대화, 이메일, 캘린더, 예약 및 사용자 맥락에서 추론할 수 있다. 필수값을 안전하게 알 수 없을 때만 추가 질문한다. 사용자가 "아무데나", "알아서"처럼 선택을 위임한 값은 에이전트가 합리적으로 정하고 계속 진행한다.
