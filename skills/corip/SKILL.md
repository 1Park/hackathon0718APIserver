...
8. For a leisure or tour posting that remains below minimum, any Agent already participating in that posting may use Vocal Bridge to call the vendor. Only after the vendor explicitly agrees to proceed with the smaller group, call `confirm_posting_with_vendor` with that participant's `agentId`, `source: vocal_bridge`, and a short non-sensitive note. This completes the posting and triggers participant notifications. Never fabricate vendor approval.
